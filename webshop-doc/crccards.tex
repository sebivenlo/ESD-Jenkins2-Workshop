\section{Classes and responsibilties}
...
\begin{crccard}
  \CRC{PriceReductionCalculator}{business-pricecalc}{
    Computes a price reduction when the customer provides a bonus
    code.}{
    When the customer enters a bonus code, a price reduction will be
    calculated.
  
  This price reduction depends on the implementation.}{
  Cart\\
  Invoice\\
  InvoiceLine
}
...
\begin{crccard}
\CRC{RequiresAgeCheck}{business}{
  To check if the customer is mature enough to purchase the products
  in the cart.
}{  A cart may contain products that may only be sold to people of a
  certain age. Products that require such checks implement this
  interface.
  
  The type is used by the invoice. The mapper decides how products are classified.
}{Mapper\\
  Invoice\\
  Cart
}
\end{crccard}
\begin{crccard}
\CRC{Booze}{business}{Example type for age check}{Booze extends
  Product implements RequiresAgeCheck}{Product\\RequiresAgeCheck}
\end{crccard}
