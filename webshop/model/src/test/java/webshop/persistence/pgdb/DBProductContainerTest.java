...
public class DBProductContainerTest {
    ...
    public void setUp() throws IOException {
        ...
        cart = new MockDBContainer( "test_carts", 1 );
    ...
    public static void teardownClass() {
        new QueryHelper().doUpdate( "drop table if exists test_inventory;\n"
                + "drop table if exists test_carts;\n" );
    }
    ...
    /**
     * Test of take method, of class PGDBProductContainer. Work:<br/>
     * Capture the itemCount of the inventory. Do NOT assume a constant qty in
     * the database Put a product in the inventory using merge. Take the product
     * from the inventory. Check if you have a new (not same) ProductyQuantity
    ...
    public void testTake() {
        ...
        int afterMargeQty = inventory.merge( p2, qty ).getQuantity();
        ProductQuantity pqTake = inventory.take( p2, 1 );
        int expectedInventory = afterMargeQty - 1;
        assertEquals( "checks and balances 1", expectedInventory,
                inventory.count( p2 ) );
        Product px = pqTake.getProduct();
        System.out.println( ">>>px = " + px );
        assertEquals( "checks and balances 2", p2, px );
