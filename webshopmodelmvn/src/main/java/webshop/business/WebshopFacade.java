...
public class WebshopFacade {
    ...
    public String getInvoiceData() {
        ...
        for ( InvoiceLine line : getInvoice().getLines() ) {
        ...
        if ( invoice.getSpecialPrice() != invoice.getTotalPriceIncludingVAT() ) {
            builder.append( String.format(
                    "SPECIAL PRICE                        %7.2f%n",
                    invoice.getSpecialPrice() / 100.0 ) );
            builder.append( String.format(
                    "YOUR ADVANTAGE WITH BONUS            %7.2f%n",
                    ( invoice.getTotalPriceIncludingVAT() - invoice
                    .getSpecialPrice() ) / 100.0 ) );
        }
